...
def run(dirs=None):
    ...
    for leaf in tqdm(all_leaves, desc="Updating metadata"):
        ...
        try:
            ...
            props = hats_catalog.catalog_info.extra_dict()

            md = catalog_data.setdefault("metadata", {})
            md["numRows"] = hats_catalog.catalog_info.total_rows
            ...
            md["sizeOnDisk"] = human_readable.file_size(
                int(props["hats_estsize"]) * 1024, binary=True
            )
            md["hatsBuilder"] = props["hats_builder"]
